...
export function OrderProvider({ children }) {
  const [orders, setOrders] = useState(() => {
    const saved = localStorage.getItem('luxebeautyO');
    return saved? JSON.parse(saved) : [];
  });
  const [appointments, setAppointments] = useState(() => {
    const saved = localStorage.getItem('luxebeautyA');
    return saved? JSON.parse(saved) : [];
  });
  ...
  useEffect(() => {
    ...
    localStorage.setItem('luxebeautyO', JSON.stringify(orders));
  }, [orders]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('luxebeautyA', JSON.stringify(appointments));
  }, [appointments]);
